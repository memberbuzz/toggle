...
;(function($){
	...
	// MBZ
	//  $.browser has been deprecated. This adds back in
	//  the necessary properties that are referred to in this library.
	$.browser={ msie: ( navigator.appName == 'Microsoft Internet Explorer') ? true : false, iphone: ( navigator.userAgent.toLowerCase().indexOf("iphone") > -1 ) }
    ...
    if( !$input.is(allow) ) return $input.find(allow).iButton(options);
		...
		this.disable = function (t){
			...
			// MBZ
			//  $.isFunction has been deprecated.
			// run callback
			if( typeof options.disable === "function" ) options.disable.apply(self, [disabled, $input, options]);
		};
		...
		this.destroy = function (){
			// MBZ
			//  unbind is deprecated.
			// remove behaviors
			$([$input[0], $container[0]]).off(".iButton");
			// MBZ
			//  unbind is deprecated.
			$(document).off(".iButton_" + id);
			...
			//  $.isFunction has been deprecated.
			// run callback
			if( typeof options.destroy === "function" ) options.destroy.apply(self, [$input, options]);
		};

		// MBZ
		//  $.trim is deprecated.
		var stringValue = options.classContainer + ' ' + options.className;
		var stringValueTrimmed = stringValue.trim();

		// MBZ
		//  There were self-closing tags here
		//  which are disallowed by v3.5 of jQuery.
    $input
			// create the wrapper code
			.wrap('<div class="' + stringValue + '" />')
    	.after(
    ...
    // automatically resize the control
		...
		// MBZ
		//  bind is deprecated.
		$container.on("mousedown.iButton touchstart.iButton", function(e) {
		...
		if( options.enableDrag ){
			...
			$(document).on("mousemove.iButton_" + id + " touchmove.iButton_" + id, function(e) {
		...
		$(document).on("mouseup.iButton_" + id + " touchend.iButton_" + id, function(e) {
			...
			if( !mouse.dragging || (((new Date()).getTime() - dragStart.time) < options.clickOffset ) ){
				...
				//  $.isFunction has been deprecated.
				// run callback
				if( typeof options.click === "function" ) options.click.apply(self, [!checked, $input, options]);
			} else {
				var x = getDragPos(e);
		...
		$input
			.on("change.iButton", function (){
				...
				//  $.isFunction has been deprecated.
				// run callback
				if( typeof options.change === "function" ) options.change.apply(self, [$input, options]);
			})
			// MBZ
			//  bind is deprecated.
			// if the element has focus, we need to highlight the container
			.on("focus.iButton", function (){
				$container.addClass(options.classFocus);
			})
			// MBZ
			//  bind is deprecated.
			// if the element has focus, we need to highlight the container
			.on("blur.iButton", function (){
		...
		if( typeof options.click === "function" ){
			// MBZ
			//  bind is deprecated.
			$input.on("click.iButton", function (){
		...
		if( $.browser.msie ){
			...
			$input.on("click.iButton", function (){ $input.triggerHandler("change.iButton"); });
		}

		// MBZ
		//  $.isFunction has been deprecated.
		// run the init callback
		if( typeof options.init === "function" ) options.init.apply(self, [$input, options]);
	};
